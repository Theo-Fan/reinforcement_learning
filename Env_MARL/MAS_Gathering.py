...
class AgentObj:
    def __init__(self, coordinates, ty, name, direction=0, mark=0, hidden=0):
        ...
        self.type = ty  # 颜色类型，0为红色，1为绿色，2为蓝色
        self.name = name  # 名称
        self.hidden = hidden  # 是否处于隐藏状态，0表示不隐藏
        self.direction = direction  # 方向：0=右，1=上，2=左，3=下
...
# 定义游戏环境类
class GameEnv:
    def __init__(self, width=31, height=11, agent_hidden=20, food_hidden=15, max_step=500):
        ...
        self.max_steps = max_step  # 每个 episode 的最大步数
        self.reset()  # 初始化环境
    ...
    def move(self, agent1_action, agent2_action):
        ...
        if not self.agent1.is_hidden():
            if agent1_action in [0, 1, 2, 3]:  # 移动类动作
                ...
                self.agent1_beam_set = self.agent1_actions[agent1_action](env_x_size=self.size_x,
                                                                          env_y_size=self.size_y)
            else:  # 旋转或停留类动作
        ...
        if not self.agent2.is_hidden():
            if agent2_action in [0, 1, 2, 3]:  # 移动类动作
                ...
                self.agent2_beam_set = self.agent2_actions[agent2_action](env_x_size=self.size_x,
                                                                          env_y_size=self.size_y)
            else:  # 旋转或停留类动作
        ...
        for food in self.food_objects:
            ...
            # if food in self.hidden_lst and food.hidden == 0:
            #     cur_all_apple = self.get_all_foods()
            #     while True:
            #         x = random.randint(1, self.size_x - 2)
            #         y = random.randint(1, self.size_y - 2)
            #         if (x, y) not in cur_all_apple:
            #             self.food_objects.remove(food)
            #             # 生成新的食物在原位置
            #             self.food_objects.append(FoodObj(coordinates=(x, y)))
            #             break

            if not food.is_hidden():  # 处理食物被吃掉的逻辑
                if not self.agent1.is_hidden() and food.x == self.agent1.x and food.y == self.agent1.y:
                    agent1_reward += food.eat(self.food_hidden)
                    self.hidden_lst.append(food)
